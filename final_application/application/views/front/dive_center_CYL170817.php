											...
											foreach ($data121 as $cval21)
												{
													...
													$logprodinfo .= "Product Leisure (<3) " . $cval21->product_name . "[" . $cval21->id . "] " . 
																	...
																	for($i = $dbegin; $i <= $dend; $i->modify('+1 day')){
																		...
																		//---------------------------Dive --------------------------
																			...
																			if ($davail < $maxdive_day) {
																				...
																				$prodcomment .= $idate . " [" . $davail . "/" . $maxdive_day . "] ";
																				
																			}
																		$logprodinfo .= "  - " . $idate . " [" . $davail . "/" . $maxdive_day . "] dd:$dd \n";
																	}
																	if($dd==0)
																	{
																...
																else
																{
																	...
																	for($i = $dbegin; $i <= $dend; $i->modify('+1 day')){
																		...
																		//---------------------------Dive --------------------------
																			...
																			$avail = $this->front_model->checkAvalability($idate,'tbl_dcleisure',$lesuiredive,$cval21->id);
																			if (is_array($avail)) {
																				$davail = $avail[0]["day_max"] - $avail[0]["booked_dives"];
																			} else {
																				$davail = $product_max_day;
								...
								<?php
										...
										if($no_of_rows < 3)
										{
												foreach ($data121 as $cval21)
												{
													...
													$maxdive_day = $cval21->max_dive_day;
													$product_max_day = $cval21->product_max_day;
													$logprodinfo .= "Product Courses (<3) " . $cval21->product_name . "[" . $cval21->id . "] " . 
														"MaxDive:" . $maxdive_day . ", ProdMax:" . $product_max_day."\n";
													$dd=0;
													?>
														<div class="row1" style="padding-bottom: 15px; border-bottom:1px solid #ccc; padding-top:20px; ">
															<button type="button" name="add_cart" class="expandLeisureDive"  data-productid="<?php echo $cval21->id; ?>" data-table="tbl_dccourses" data-diveid="<?php echo $lesuiredive; ?>"  style="background-color: #fff;border: none;width: 100%;"/>
																...
																else
																{
																	...
																	for($i = $dbegin; $i <= $dend; $i->modify('+1 day')){
																		$ddiff++;
																		$idate = $i->format("Y-m-d");
																		$avail = $this->front_model->checkAvalability($idate,'tbl_dccourses',$lesuiredive,$cval21->id);
																		if (is_array($avail)) {
																			$davail = $avail[0]["day_max"] - $avail[0]["booked_dives"];
																		} else {
																			$davail = $product_max_day;
															...
															<button type="button" name="add_cart" class="expandLeisureDive"  data-productid="<?php echo $cval21->id; ?>" data-table="tbl_dcpackage" data-diveid="<?php echo $lesuiredive; ?>"  style="background-color: #fff;border: none;width: 100%;"/>
																...
																else
																{
																	...
																	for($i = $dbegin; $i <= $dend; $i->modify('+1 day')){
																		$ddiff++;
																		$idate = $i->format("Y-m-d");
																		$avail = $this->front_model->checkAvalability($idate,'tbl_dcpackage',$lesuiredive,$cval21->id);
																		if (is_array($avail)) {
																			$davail = $avail[0]["day_max"] - $avail[0]["booked_dives"];
																		} else {
																			$davail = $product_max_day;
																	...
																	{
																	$logprodinfo .= "  - Listed available with dd=$dd \n";
																	$logprodinfo .= " Booking period " . $cval21->booking_period_start_date . " to " . $cval21->booking_period_end_date . " "
																			. ", Travel period " . $cval21->travel_period_start_date . " to " . $cval21->booking_period_end_date;
																	$bookdate = new DateTime();
																	if (($bookdate >= $cval21->booking_period_start_date) && ($bookdate <= $cval21->booking_period_end_date)) {
																		$logprodinfo .= " Booking within! "; 
																	} else {
																		$logprodinfo .= " Booking outside! "; 
																	}
																	//if ($cval21->booking_period_start_date
																	if (($dbegin >= $cval21->travel_period_start_date) && ($dbegin <= $cval21->travel_period_end_date)) {
																...
																<?php
																	...
																	<div class="col-md-3">
																	<a class="" data-toggle="collapse" data-parent="#accordion" href="#<?php echo $cval21->id; ?>" style="color:red;font-weight:bold;">UNAVAILABLE</a> <br>
																	</div>
																	<div class="col-md-3"><?php echo $prodcomment; ?></div>
																...
																?>
																<div class="col-md-3">
																	<a class="" data-toggle="collapse" data-parent="#accordion" href="#<?php echo $cval21->id; ?>"><b><?php echo ucwords(strtoupper($divecenterval->dccurrency));?>&nbsp;&nbsp;<?php echo $newamt[$t]; ?></b>&nbsp;&nbsp;Per&nbsp;&nbsp;<?php echo $cval21->product_unit;?></a>
																</div>
																<?php
																		}
																$t++;
