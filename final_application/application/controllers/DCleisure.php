  ...
  function enable($id)
  {
	...
	foreach($data1 as $data1_row)
	{
		...
		$data2 = array(
			...
			'discount_bulk_purchase_amount' => $data1_row->discount_bulk_purchase_amount,
			...
			'ap_discount_amount' => $data1_row->ap_discount_amount,
			'apply_promo_bulk_discount' => $data1_row->apply_promo_bulk_discount,
			'apb_amount' => $data1_row->apb_amount,
...
//**********************************************************************************************************************************************//
 ...
 {
     $this->db->where('id',$id);
     if($this->db->update('tbl_dcleisure',array('status' => 'ENABLE')))
     {
         $data['DCleisuredashboard']=$this->DCleisuremodel->DCleisuredashboard();
    $this->load->view('template/header');
    $this->load->view('template/sidebar');
