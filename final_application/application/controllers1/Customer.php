...
*/		
	...
	public function chk_promo()
	{
		...
		$sql ="SELECT * FROM `promo_code_details` WHERE `promo_code` = '$promo_code' AND `currency`= '$currency'     AND `status` = 1 AND (`valid_from` >= '$date' OR `valid_to` >= '$date' ) ";
	...
	Public function check_promocode()
	{
		...
		//$promo_code = 'F3EFNS6';
	 	...
	 	$sql ="SELECT * FROM `promo_code_details` WHERE `promo_code` = '$promo_code' AND `currency`= '$currency' `status` = 1 AND (`valid_from` >= '$date' OR `valid_to` >= '$date' ) ";
		$data = $this->db->query($sql)->row();
		$result = ($data)?$data:array();
		echo json_encode($result);
