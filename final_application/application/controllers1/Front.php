	...
	function pre_divecenter($id,$sDate,$eDate,$no_of_persons)
	{
		...
		if($dbdata_num > 0)
		{
			//data alraedy find.
				...
				redirect('front/divecenter/'.$id.'/'.$sDate.'/'.$eDate.'/'.$no_of_persons);
		}
		else
		{
			if($month == 01)
...
$amount =0;
		...
		$result1 = $this->currencyConversation($data , $daytype , '1' ,$insert['noofPerson'],'courseRoomrates');
...
$amount=0;
		...
		$result1 = $this->currencyConversation($data , $daytype , '1' ,$insert['noofPerson'],'packageRoomrates');
					...
					//'no_of_rooms'=>$no_of_rooms_person
								...
								if ($this->input->post('accom') == "Yes") {
									...
									if($this->input->post('table_name') == 'tbl_dccourses')
									{
										...
										$r12 = $this->currencyConversation($resrm , $this->input->post('daytype') , $this->input->post('accommodation_type') , $this->input->post('no_of_persons') ,'courseRoomrates');
										$rmrate = $r12['disprice'];
									}
									else
									{
									...
									$r12 = $this->currencyConversation($resrm , $this->input->post('daytype') , $this->input->post('accommodation_type') , $this->input->post('no_of_persons'),'packageRoomrates');
										$rmrate = $r12['disprice'];
									}
									/* if ($qrm->num_rows() > 0)
									{
...
// if promo is not applicable means next check the Bulk discount
	...
	function currencyConversation($data , $daytype , $accom_type , $no_of_persons , $functionType)
	{
		$result = "";
		$newAmount = "";
	...
	function contact_mail()
	{
		if($this->input->post('submit_contact_mail'))
		{
				...
				$Message = $this->input->post('contact_message');
					...
					$result = $this->email
							->from('info@scubbi.com')
							->reply_to('info@scubbi.com')    // Optional, an account where a human being reads.
							->to('info@scubbi.com')
							->subject($subject)
							->message($body)
							->send();
